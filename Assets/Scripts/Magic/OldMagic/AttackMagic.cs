...
public class AttackMagic : Magic
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Enemy"))
        {
            ...
            if (isNormal || isTrack)
            {
                if (collision.GetComponent<WizardController>().GetDamage(damage))
                {
                    //造成伤害
    ...
    public void MagicBurst()
    {
        ...
        foreach (var collider in Physics2D.OverlapCircleAll(transform.position, burstRadius))
        {
            ...
            {
                if (collider.GetComponent<WizardController>().GetDamage(burstDamage * curAttachValue))
                {
                ...
                {
                    collider.GetComponent<WizardController>().OpenDefenseEffect(transform, true);
                }
            }
        }
        //魔力爆发结束后销毁当前脚本
